...
require("CopilotChat").setup {
  ...
  prompts = {
    ...
    Credo = {
      prompt = 'Run `mix credo` and fix all issues.',
      system_prompt = COPILOT_CREDO,
      context = 'file:.copilot/credo.md',
      mapping = '<leader>cr',
      progress = function()
        return false
      end,
      callback = function(response, source)
        chat.chat:append('Credo issues fixed successfully!', source.winnr)
        local credo_file = source.cwd() .. '/.copilot/credo.md'
        local dir = vim.fn.fnamemodify(credo_file, ':h')
        vim.fn.mkdir(dir, 'p')
        local file = io.open(credo_file, 'w')
        if file then
          file:write(response)
          file:close()
        end
